...
class Model:
    def __init__( self, n_feats, hidden_layers, n_classes ):
        ...
        with self.graph.as_default():
            ...
            #TODO Add dropout
            self.fw_lstm_stack = tf.contrib.rnn.MultiRNNCell( list( map( self.lstm_cell, self.hidden_layers ) ) ) #Forward
            self.bw_lstm_stack = tf.contrib.rnn.MultiRNNCell( list( map( self.lstm_cell, self.hidden_layers ) ) ) #Backward
            self.rnn_outs, _ = tf.nn.bidirectional_dynamic_rnn( self.fw_lstm_stack, self.bw_lstm_stack, self.x,\
                    self.seq_len_vector, dtype=tf.float32 )
